...
int main(void){
    ...
    vecD[0] = 1.0;
    for (m = 1;m<N+1;++m){/*ringの場合はA/2 + A/2で等しい。t/2で分割しなくて良い。横と下はできてたsquareで*/
        for (i = 0;i<l;++i){
            for (j = 0; j < E; ++j){
                C = vecB[j];/*同じBC*/
                ...
                if(j % d == d-1){/*右BA*/
                    C = vecB[j];
                    vecB[j] = C * cos(tau*v/2) - 1.0I * vecA[j - (d-1)] * sin(tau*v/2);
                    vecA[j - (d-1)] = vecA[j - (d-1)] * cos(tau*v/2) - 1.0I * C * sin(tau*v/2);
                    /*printf("右A%d \n",j - (d-1));*/
                }else{
                    C = vecB[j];
                    vecB[j] = C * cos(tau*v/2) - 1.0I * vecA[(j+1) % E] * sin(tau*v/2);
                    vecA[(j+1) % E] = vecA[(j+1) % E] * cos(tau*v/2) - 1.0I * C * sin(tau*v/2);
                    /*printf("右A%d \n",(j+1) % E);*/
                }
            }
